...
const { default: init } = await import('../../../blocks/forge-new-block-5/forge-new-block-5.js');

describe('forge-new-block-5 (hidden alternate)', () => {
  before(async () => {
    document.body.innerHTML = await readFile({ path: './mocks/body.html' });
  });

  it('marks the block hidden and aria-hidden', async () => {
    const block = document.querySelector('.forge-new-block-5');
