...
/**
 * Collect top-level <script> elements from the template. Call this before
 * main swap; the collected elements are appended after main.innerHTML is set
 * so the scripts see the fully-populated DOM.
 */
function collectTemplateScripts(templateDoc, base) {
  const pending = [];
  templateDoc.body.querySelectorAll(':scope > script').forEach((s) => {
    const el = document.createElement('script');
    const src = s.getAttribute('src');
    if (src) {
      // Resolve root-relative paths against the code base
      el.src = src.startsWith('http') ? src : `${base}${src}`;
    } else {
...
export default async function decorate(block) {
  ...
  // Collect scripts BEFORE main swap; they are appended after the swap
  // so the scripts execute against the fully-populated DOM.
  const pendingScripts = collectTemplateScripts(doc, base);
  ...
  // Append per-template scripts AFTER main is populated. Scripts in innerHTML
  // do not execute (browser security), so we inject them separately here.
  // They see the live, fully-populated DOM.
  pendingScripts.forEach((s) => document.head.appendChild(s));
}
