...
function validateEnv() {
  if (!AEM_LIVE_ADMIN_TOKEN || !ORG || !REPO || !ROLLING_IMPORT_SLACK) {
    throw new Error(
      `Missing env variable. AEM_LIVE_ADMIN_TOKEN: ${!!AEM_LIVE_ADMIN_TOKEN}, ORG: ${!!ORG}, REPO: ${!!REPO}, ROLLING_IMPORT_SLACK: ${!!ROLLING_IMPORT_SLACK}`
    );
  }
}
...
async function getLivePaths(entries) {
  ...
  if (!LOCAL_RUN)
    await slackNotification(
      `Importing ${livePaths.length} published documents from ${entries.length} log entries. Log Link: https://admin.hlx.page/log/${ORG}/${REPO}?from=${FROM_PARAM}`,
      ORG,
      REPO
    );
  if (livePaths.length < 10 && !LOCAL_RUN) console.log("First 10 paths to import:\n" + livePaths.slice(0, 10).join('\n'));
  return livePaths;
}
...
async function main() {
  ...
  const entries = await fetchLogsForSite(
    REPO,
    `https://admin.hlx.page/log/${ORG}/${REPO}`,
    FROM_PARAM
  );
  ...
  for (const path of livePaths) {
    queue.add(() =>
      importUrl(path, importedMedia)
        .then(() => {
          result.success++
          if (result.success % 20 === 0) console.log(`Progress: Success: ${result.success} | Error: ${result.error}`)
        })
        .catch(() => {
          if (result.error % 10 === 0) console.log(`Progress: Success: ${result.success} | Error: ${result.error}`)
          result.error++;
