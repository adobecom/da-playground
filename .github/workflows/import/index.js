...
const ENABLE_DEBUG_LOGS = process.env.ENABLE_DEBUG_LOGS;
...
async function importMedia(pageUrl, text, importedMedia) {
  if (importedMedia.has(pageUrl.pathname)) {
    if (ENABLE_DEBUG_LOGS) console.log(`Skipping ${pageUrl.pathname} as it's already imported`);
    return
  }
  ...
  for (const mediaUrl of linkedMedia) {
    if (ENABLE_DEBUG_LOGS) console.log(`Importing media: ${mediaUrl.href}`);
    try {
      await importUrl(mediaUrl.pathname, importedMedia);
      importedMedia.add(mediaUrl.pathname)
    } catch (error) {
      if (ENABLE_DEBUG_LOGS) console.log({msg: error.message})
    }
  }
...
async function saveAllToDa(url, blob) {
  ...
  try {
    ...
    if (ENABLE_DEBUG_LOGS) console.log(`Couldn't save ${destPath}`);
    return 500;
  }
}
...
async function previewOrPublish({path, action}) {
  ...
  if(resp.ok && ENABLE_DEBUG_LOGS) console.log(`Posted to ${action} successfully ${action}/${toOrg}/${toRepo}/main${path}`);
  if(resp.ok) return

  if (ENABLE_DEBUG_LOGS) console.log(`Posting to ${action} failed. ${action}/${toOrg}/${toRepo}/main${path}`);
  throw new Error(`Failed ${action}/${toOrg}/${toRepo}/main${path}. Error: ${resp.status} ${resp.statusText}`)
}
...
async function importUrl(aemPath, importedMedia) {
  ...
  if(excludedFiles.some((excludedFile => url.pathname === excludedFile))) {
    if (ENABLE_DEBUG_LOGS) console.log(`Stopped processing ${url.pathname}`);
    return
  }

  if (ENABLE_DEBUG_LOGS) console.log("Started path: ", url.href);
  const [fromRepo, fromOrg] = url.hostname.split('.')[0].split('--').slice(1).slice(-2);
  if (!(fromRepo || fromOrg)) {
    if (ENABLE_DEBUG_LOGS) console.log(liveDomain, url.origin === liveDomain);
  ...
  try {
    ...
    if(resp.status === 404) {
      if (ENABLE_DEBUG_LOGS) console.log("Resource not found: ", `${url.origin}${srcPath}`)
      return
    }
    if (ENABLE_DEBUG_LOGS) console.log("fetched resource from AEM at:", `${url.origin}${srcPath}`)
    if (resp.redirected && !(srcPath.endsWith('.mp4') || srcPath.endsWith('.png') || srcPath.endsWith('.jpg'))) {
      url.status = 'redir';
      if (ENABLE_DEBUG_LOGS) console.log("Skipped importing redirected resource")
      return
    }
    if (!resp.ok && resp.status !== 304) {
      url.status = 'error';
      if (ENABLE_DEBUG_LOGS) console.log(`Failed Status ${resp.status} /${toOrg}/${toRepo}/main${path}. Error: ${resp.status} ${resp.statusText}`)
      throw new Error(`Failed Status ${resp.status} /${toOrg}/${toRepo}/main${path}. Error: ${resp.status} ${resp.statusText}`)
    }
    ...
    if (ENABLE_DEBUG_LOGS) console.log("imported resource to DA " + url.daHref);
    await previewOrPublish({path: pathname, action: 'preview'});
    await previewOrPublish({path: pathname, action: 'live'});
  } catch (e) {
    if (ENABLE_DEBUG_LOGS) console.log("Failed to import resource to DA " + toOrg + "/" + toRepo + " | destination: " + url.pathname + " | error: " + e.message);
    throw e;
  }
}
