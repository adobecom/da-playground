# build-block-from-figma — task: forge-section-11
...
- **Block name:** `forge-section-11`
- **Target path:** `/Users/cod87753/Code/da-playground/.forge-worktrees/session-f5c1aa4f/libs/c2/blocks/forge-section-11` (NEVER libs/blocks/)
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-11` /
  `.forge-section-11`. Use as the starting point for `forge-section-11.css`; prune rules that don't apply.
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/cod87753/Code/da-playground/.forge-worktrees/session-f5c1aa4f/libs/c2/blocks/forge-section-11/forge-section-11.js` (`export default
   async function init(el)`) and `forge-section-11.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-section-11.test.js`) so forge-block-lint passes.
