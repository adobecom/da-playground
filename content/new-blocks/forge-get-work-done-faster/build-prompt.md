# build-block-from-figma — task: forge-get-work-done-faster
...
- **Block name:** `forge-get-work-done-faster`
- **Target path:** `/Users/bradjohn/Documents/da-playground/.forge-worktrees/session-8007fcd6-ms67m882/libs/c2/blocks/forge-get-work-done-faster` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-8007fcd6-33a` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/local-dev/forge/figma-8007fc?milolibs=local` — run Phase 5 in
  ...
  `?milolibs=` delivery.
- **Figma URL:** https://www.figma.com/design/fMKQndDRSNu4yTjDYyiQoN/Home-%E2%80%94-A.com?node-id=8473-17503
- **Figma node id:** 8473:17504
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-0` /
  `.forge-get-work-done-faster`. Use as the starting point for `forge-get-work-done-faster.css`; prune rules that don't apply.
...
- `page-globals.css` — page-level globals (:root tokens, resets, @font-face)
  ...
  (do NOT inline these into `forge-get-work-done-faster.css` — link them from the page head).
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/bradjohn/Documents/da-playground/.forge-worktrees/session-8007fcd6-ms67m882/libs/c2/blocks/forge-get-work-done-faster/forge-get-work-done-faster.js` (`export default
   async function init(el)`) and `forge-get-work-done-faster.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-get-work-done-faster.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-8007fcd6-33a` for
