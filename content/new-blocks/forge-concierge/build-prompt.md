# build-block-from-figma — task: forge-concierge
...
- **Block name:** `forge-concierge`
- **Target path:** `/Users/cod87753/Code/da-playground/.forge-worktrees/session-7d1abf2e-mqjtty3w/libs/c2/blocks/forge-concierge` (NEVER libs/blocks/)
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-10` /
  `.forge-concierge`. Use as the starting point for `forge-concierge.css`; prune rules that don't apply.
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/cod87753/Code/da-playground/.forge-worktrees/session-7d1abf2e-mqjtty3w/libs/c2/blocks/forge-concierge/forge-concierge.js` (`export default
   async function init(el)`) and `forge-concierge.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-concierge.test.js`) so forge-block-lint passes.
...
- R3 matcher decision: `tight-variant` (score: 0.85)
- Closest existing C2 block: `brand-concierge`

If the score is close to a tight/loose-variant threshold, consider whether
this should instead become a variant proposal on `brand-concierge`
