# build-block-from-figma — task: forge-hero
...
- **Block name:** `forge-hero`
- **Target path:** `/Users/victor/dev/da-playground/.forge-worktrees/session-5cbb7c4f/libs/c2/blocks/forge-hero` (NEVER libs/blocks/)
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-0` /
  `.forge-hero`. Use as the starting point for `forge-hero.css`; prune rules that don't apply.
...
- `page-globals.css` — page-level globals (:root tokens, resets, @font-face)
  ...
  (do NOT inline these into `forge-hero.css` — link them from the page head).
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/victor/dev/da-playground/.forge-worktrees/session-5cbb7c4f/libs/c2/blocks/forge-hero/forge-hero.js` (`export default
   async function init(el)`) and `forge-hero.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-hero.test.js`) so forge-block-lint passes.
...
- R3 matcher decision: `tight-variant` (score: 0.91)
