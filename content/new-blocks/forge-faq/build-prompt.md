# build-block-from-figma — task: forge-faq
...
- **Block name:** `forge-faq`
- **Target path:** `/Users/adambaker/source/da-playground/.forge-worktrees/session-14818c30/libs/c2/blocks/forge-faq` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-14818c30-61c` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/adamb/forge/variant-a-14818c?milolibs=local` — run Phase 5 in
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-6` /
  `.forge-faq`. Use as the starting point for `forge-faq.css`; prune rules that don't apply.
...
- `page-globals.css` — page-level globals (:root tokens, resets, @font-face)
  ...
  (do NOT inline these into `forge-faq.css` — link them from the page head).
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/adambaker/source/da-playground/.forge-worktrees/session-14818c30/libs/c2/blocks/forge-faq/forge-faq.js` (`export default
   async function init(el)`) and `forge-faq.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-faq.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-14818c30-61c` for
