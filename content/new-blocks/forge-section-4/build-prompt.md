# build-block-from-figma — task: forge-section-4
...
- **Block name:** `forge-section-4`
- **Target path:** `/Users/cod87753/Code/da-playground/.forge-worktrees/session-f5c1aa4f/libs/c2/blocks/forge-section-4` (NEVER libs/blocks/)
...
- `truth-target.css` — **VISUAL TARGET, REFERENCE ONLY — do NOT copy it.** This is the Figma section's CSS re-scoped to `.forge-section-4`. Its selectors describe the layout/grid/gaps/colour/type INTENT to reproduce, but its class names (e.g. `.usecase`, `.bento`, `.appicon`) DO NOT EXIST in `author-content.html` — porting it verbatim renders the block UNSTYLED. TRANSLATE its intentions into rules keyed on the classes YOUR `decorate()` stamps on the reconstructed DOM.
...
- `starter.css` — the section's CSS, scoped under `.sf-sec-4` /
  `.forge-section-4`. For this DISTINCTIVE section it is a minimal STRUCTURAL FLOOR (keyed on `.foreground`/`.media`/`.content`) — replace it with CSS you author toward `truth-target.css` + the screenshot.
...
init(el)  ->  PROBE by content/order  ->  RECONSTRUCT rich DOM
              (createTag/createElement + append, in document order;
               add YOUR OWN .forge-section-4-scoped classes via classList.add)
        v
scoped .forge-section-4 CSS  ->  styles ONLY the classes init() just stamped
        v
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/cod87753/Code/da-playground/.forge-worktrees/session-f5c1aa4f/libs/c2/blocks/forge-section-4/forge-section-4.js` (`export default
   async function init(el)`) and `forge-section-4.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-section-4.test.js`) so forge-block-lint passes.
