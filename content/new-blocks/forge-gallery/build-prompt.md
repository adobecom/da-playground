# build-block-from-figma — task: forge-gallery
...
- **Block name:** `forge-gallery`
- **Target path:** `/Users/victor/dev/da-playground/.forge-worktrees/session-5cbb7c4f/libs/c2/blocks/forge-gallery` (NEVER libs/blocks/)
- **Milo branch:** `forge/session-5cbb7c4f-a0b` (cut off forge-a-panel)
- **Base branch:** `forge-a-panel`
- **Preview URL (LOCALHOST mode):** `http://localhost:3000/drafts/vhargrave/forge/hub-acom-seeded-html-5cbb7c?milolibs=local` — run Phase 5 in
...
- `section.html` — the faithful section's outerHTML WITH its Figma descriptive classes. Use it ONLY as a layout REFERENCE for grouping/intent; **its classes are stripped before `init()` runs**, so never key CSS or probes on them. Author against `author-content.html` (the real runtime shape), not this. **DIMENSIONS ARE NOT CLASSES:** the proportions `section.html` encodes — per-item heights/widths/aspect-ratios, column/row counts, gaps, and order — ARE the design and are lost from the runtime content. Read the ACTUAL sequence of those values out of `section.html` and preserve it in YOUR block (a positional array in JS, or `:nth-child`/index-keyed rules in CSS, keyed on the DOM `init()` builds). NEVER invent, cycle, or approximate a proportion/cadence that `section.html` already specifies — transcribe the real values.
- `starter.css` — the section's CSS, scoped under `.sf-sec-1` /
  `.forge-gallery`. Use as the starting point for `forge-gallery.css`; prune rules that don't apply.
...
2. Phase 1: gather the Figma context via the Figma MCP if the URL is set;
   ...
   missing Figma URL).
3. Phase 4: author `/Users/victor/dev/da-playground/.forge-worktrees/session-5cbb7c4f/libs/c2/blocks/forge-gallery/forge-gallery.js` (`export default
   async function init(el)`) and `forge-gallery.css`. Register in
   `C2_BLOCKS` at `libs/utils/utils.js:111`. Emit the L22 test fixture
   (`mocks/body.html` + `forge-gallery.test.js`) so forge-block-lint passes.
4. Phases 5–7: Playwright visual match against section.html (LOCALHOST mode),
   axe a11y, Lighthouse perf.
5. Phase 8: Forge owns the push — leave the branch `forge/session-5cbb7c4f-a0b` for
   shipBlocksToMilo to commit + push and deliver via `?milolibs=`.

## Decision provenance

- R3 matcher decision: `ambiguous` (score: 0.70)
