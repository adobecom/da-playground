...
function buildPipelineHtmlToDA() {
  ...
  const genStep = makeStep('gen-html', 'Generate HTML');
  const lineA = makeLine();
  const mapStep = makeStep('slice-map', 'Slice & map sections');
  const lineB = makeLine();
  const assembleStep = makeStep('assemble', 'Assemble document');
  const lineC = makeLine();
  const uploadStep = makeStep('upload', 'Upload & preview');

  dag.append(genStep, lineA, mapStep, lineB, assembleStep, lineC, uploadStep);
  ...
  function update(job) {
    ...
    setStep(mapStep, stage < 1 ? 'pending' : stage === 1 ? 'running' : 'done');
    lineB.className = 'pipe-line' + (stage > 1 ? ' pipe-line--done' : stage === 1 ? ' pipe-line--active' : '');
    setStep(assembleStep, stage < 2 ? 'pending' : stage === 2 ? 'running' : 'done');
    lineC.className = 'pipe-line' + (stage > 2 ? ' pipe-line--done' : stage === 2 ? ' pipe-line--active' : '');
    setStep(uploadStep, stage < 3 ? 'pending' : isDone ? 'done' : 'running');

    if (isDone) {
      setStep(genStep, 'done'); setStep(mapStep, 'done');
      setStep(assembleStep, 'done'); setStep(uploadStep, 'done');
      lineA.className = 'pipe-line pipe-line--done';
      lineB.className = 'pipe-line pipe-line--done';
      lineC.className = 'pipe-line pipe-line--done';
    }
    if (isError) {
      const cur = uploadStep.dataset.status;
      if (cur === 'running' || cur === 'pending') setStep(uploadStep, 'error');
