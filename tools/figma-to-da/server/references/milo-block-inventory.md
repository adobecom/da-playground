# Milo Block Inventory — Standard Library

These blocks are provided by Milo (`libs/blocks/`) and are available on any upp page without creating any new block files. They render with Milo's default styling.
...
**Use when:** Full-width hero banner with a large heading, body text, CTAs, and optional background image or video.
...
**Use when:** A side-by-side layout with N equal or custom-width columns, where each column contains arbitrary mixed content (text, image, list, links). Use `columns` when there are 3+ columns or when content is not the standard text+image pair.
...
**Use when:** A pull-quote or testimonial where attribution is on the same line or immediately below.
...
**Use when:** A standalone image or video component, optionally with a heading and text. Used for product screenshots, demo videos, or large feature images that stand alone.
